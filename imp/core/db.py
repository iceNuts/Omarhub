...
class dbmgr:
	...
	def get_following_brief_list(self,mail,num):
		"""Get the a brief list of my following"""
		self.create_db_connection()
		if not mail:
			return None
		result = self.db.query("select u.user_id,u.first_name,u.last_name,u.avatar from Follow_Status f,Users u where f.mail_from=%s and f.mail_to=u.mail limit 0,%s", mail,num)
		self.drop_db_connection()
		return result
